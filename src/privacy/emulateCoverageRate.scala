...
object emulateCoverageRate {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val threads = Array(new Thread(new compositionEstimator(tryNum, sampleSize, R, epsilon, r_ind)))
...
//    val threads = (for (lambda1 <- Array.range(20, 26)) yield {
//      new Thread(new privacy.twoStageEstimator(tryNum, sampleSize, R, epsilon, r_ind, Array(2), lambda1, 10))
    ...
    for (k <- 10 to 10) {
      ...
      for (w <- 0 until tryNum) {
        ...
        val dummies = for (i <- Array.range(0, k)) yield {
          val r = sigma(k) * sqrt(-2 * log(1 - random()))
